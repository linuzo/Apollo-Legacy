...
class MakeServerCommand extends VanillaCommand{
	...
	public function execute(CommandSender $sender, $commandLabel, array $args){
		...
		$pharPath = Server::getInstance()->getPluginPath() . DIRECTORY_SEPARATOR . "DarkSystem" . DIRECTORY_SEPARATOR . $server->getName() . "_" . $server->getPocketMineVersion() . ".phar";
		...
		$phar->setMetadata([
			"name" => $server->getName(),
			"version" => $server->getDarkSystemVersion(),
		...
		$sender->sendMessage($server->getName() . " " . $server->getDarkSystemVersion() . " Phar file has been created on " . $pharPath);
