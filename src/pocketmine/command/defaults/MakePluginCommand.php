...
class MakePluginCommand extends VanillaCommand {
	...
	public function __construct($name){
		parent::__construct(
			$name,
			"Creates a Phar plugin from folder Structure",
	...
	public function execute(CommandSender $sender, $commandLabel, array $args){
		...
		if($description->getName() === "DevTools"){
			...
			$phar->setStub('<?php echo "Plugin ' . $description->getName() . ' v' . $description->getVersion() . '\nThis plugin has been compiled using spigotpe at ' . date("r") . '\n----------------\n";if(extension_loaded("phar")){$phar = new \Phar(__FILE__);foreach($phar->getMetadata() as $key => $value){echo ucfirst($key).": ".(is_array($value) ? implode(", ", $value):$value)."\n";}} __HALT_COMPILER();');
		}
