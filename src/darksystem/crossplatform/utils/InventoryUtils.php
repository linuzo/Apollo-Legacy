...
use pocketmine\network\protocol\InventoryContentPacket;
use pocketmine\network\protocol\InventorySlotPacket;
...
class InventoryUtils{
	 ...
	 * @return Item[]
	 */
	public function getHotbar($items){
	 ...
	 */
	private function &getItemAndSlot($windowId, $inventorySlot, &$targetWindowId = null, &$targetInventorySlot = null){
	 ...
	 */
	private function dropCraftingItem(&$craftingItem){
	 ...
	 */
	public function onWindowOpen(ContainerOpenPacket $packet){
		$type = "";
		switch($packet->type){
	 ...
	 */
	public function onWindowCloseFromPCtoPE(ClientCloseWindowPacket $packet){
	 ...
	 */
	public function onWindowCloseFromPEtoPC(ContainerClosePacket $packet){
		$this->dropHeldItem();
	 ...
	 * @return OutboundPacket|null
	 */
	public function onWindowSetSlot(InventorySlotPacket $packet){
		$pk = new SetSlotPacket();
		$pk->windowID = $packet->windowId;
	 ...
	 */
	public function onWindowSetData(ContainerSetDataPacket $packet){
		if(!isset($this->windowInfo[$packet->windowId])){
		...
		switch($this->windowInfo[$packet->windowId]["type"]){
			case WindowTypes::FURNACE:
				switch($packet->property){
					case ContainerSetDataPacket::PROPERTY_FURNACE_TICK_COUNT:
	 ...
	 * @return OutboundPacket[]
	 */
	public function onWindowSetContent(InventoryContentPacket $packet){
		$packets = [];
	 ...
	 */
	public function onWindowClick(ClickWindowPacket $packet){
	 ...
	 */
	public function onCreativeInventoryAction(CreativeInventoryActionPacket $packet){
	 ...
	 * @return OutboundPacket|null
	 */
	public function onTakeItemEntity(TakeItemEntityPacket $packet){
		$itemCount = 1;
		$item = Item::get(0);
	 ...
	 * @param int $windowId
	 */
	public function onCraft($windowId){
		...
		if(!is_null($saveInventoryData)){
			foreach($saveInventoryData as $slot => $item){
				...
				$gridItem = clone $item;
				$inputSlotMap[$y][$x] = $gridItem->setCount(1);
			}
		}
	 ...
	 */
	public function addNetworkInventoryAction($sourceType, $windowId, $inventorySlot, Item $oldItem, Item $newItem){
	 ...
	 * @return bool
	 */
	public function checkInventoryTransactionPacket(InventoryTransactionPacket $packet){
		$errors = 0;
		$actions = [];
