	...
	public function __construct(Server $server, string $path){
		...
		if(Translate::checkTurkish() === "yes"){
			if(!file_exists($this->path . "doku_paketleri.yml")){
				file_put_contents($this->path . "doku_paketleri.yml", file_get_contents($this->server->getFilePath() . "src/darksystem/resources/doku_paketleri.yml"));
			}
			
			$this->resourcePacksConfig = new Config($this->path . "doku_paketleri.yml", Config::YAML, []);
		}else{
			if(!file_exists($this->path . "resource_packs.yml")){
				file_put_contents($this->path . "resource_packs.yml", file_get_contents($this->server->getFilePath() . "src/darksystem/resources/resource_packs.yml"));
			}
