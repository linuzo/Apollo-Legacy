...
class RSA
{
    ...
    */
     ...
     * Raw public key
     *
     * An array containing two \phpseclib\Math\Integer objects.
     *
    ...
    function __construct()
    {
        ...
        if (!defined('CRYPT_RSA_MODE')) {
            switch (true) {
                // Math/Integer's openssl requirements are a little less stringent than Crypt/RSA's. in particular,
                // Math/Integer doesn't require an openssl.cfg file whereas Crypt/RSA does. so if Math/Integer
    ...
    function createKey($bits = 1024, $timeout = false, $partial = array())
    {
        ...
        static $e;
        if (!isset($e)) {
            $e = new Integer(CRYPT_RSA_EXPONENT);
        }
        ...
        switch ($type) {
            ...
            case self::PUBLIC_FORMAT_PKCS1:
                ...
                if (strlen($temp) != 1 || ord($temp) > 2) {
                    $components['modulus'] = new Integer($temp, 256);
                    $this->_string_shift($key); // skip over self::ASN1_INTEGER
                    $length = $this->_decodeLength($key);
                    $components[$type == self::PUBLIC_FORMAT_PKCS1 ? 'publicExponent' : 'privateExponent'] = new Integer($this->_string_shift($key, $length), 256);
                ...
                $length = $this->_decodeLength($key);
                $components['modulus'] = new Integer($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['publicExponent'] = new Integer($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['privateExponent'] = new Integer($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['primes'] = array(1 => new Integer($this->_string_shift($key, $length), 256));
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['primes'][] = new Integer($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['exponents'] = array(1 => new Integer($this->_string_shift($key, $length), 256));
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['exponents'][] = new Integer($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['coefficients'] = array(2 => new Integer($this->_string_shift($key, $length), 256));
                ...
                if (!empty($key)) {
                    ...
                    while (!empty($key)) {
                        ...
                        $this->_string_shift($key);
                        $length = $this->_decodeLength($key);
                        $components['coefficients'][] = new Integer($this->_string_shift($key, $length), 256);
                    }
                }
            ...
            case self::PUBLIC_FORMAT_OPENSSH:
                ...
                $publicExponent = new Integer($this->_string_shift($key, $length), -256);
                if (strlen($key) <= 4) {
                    return false;
                }
                extract(unpack('Nlength', $this->_string_shift($key, 4)));
                $modulus = new Integer($this->_string_shift($key, $length), -256);
                ...
                if ($cleanup && strlen($key)) {
                    ...
                    extract(unpack('Nlength', $this->_string_shift($key, 4)));
                    $realModulus = new Integer($this->_string_shift($key, $length), -256);
            ...
            case self::PRIVATE_FORMAT_PUTTY:
                ...
                $components['publicExponent'] = new Integer($this->_string_shift($public, $length), -256);
                extract(unpack('Nlength', $this->_string_shift($public, 4)));
                $components['modulus'] = new Integer($this->_string_shift($public, $length), -256);
                ...
                $components['privateExponent'] = new Integer($this->_string_shift($private, $length), -256);
                extract(unpack('Nlength', $this->_string_shift($private, 4)));
                if (strlen($private) < $length) {
                    return false;
                }
                $components['primes'] = array(1 => new Integer($this->_string_shift($private, $length), -256));
                extract(unpack('Nlength', $this->_string_shift($private, 4)));
                if (strlen($private) < $length) {
                    return false;
                }
                $components['primes'][] = new Integer($this->_string_shift($private, $length), -256);
                ...
                $components['coefficients'] = array(2 => new Integer($this->_string_shift($private, $length), -256));
    ...
    function _stop_element_handler($parser, $name)
    {
        if (isset($this->current)) {
            $this->current = new Integer(base64_decode($this->current), 256);
            unset($this->current);
        }
    }
    ...
    function _generateMinMax($bits)
    {
        ...
        return array(
            'min' => new Integer($min, 256),
            'max' => new Integer($max, 256)
        );
    }
     ...
     * @access private
     * @param \phpseclib\Math\Integer $x
     * @param \phpseclib\Math\Integer $r
     * @param int $i
     * @return \phpseclib\Math\Integer
     */
     ...
     *
     * @access private
     * @param \phpseclib\Math\Integer $m
     * @return \phpseclib\Math\Integer
     */
    function _rsaep($m)
    {
     ...
     *
     * @access private
     * @param \phpseclib\Math\Integer $c
     * @return \phpseclib\Math\Integer
     */
    function _rsadp($c)
    {
     ...
     *
     * @access private
     * @param \phpseclib\Math\Integer $s
     * @return \phpseclib\Math\Integer
     */
    function _rsavp1($s)
    {
