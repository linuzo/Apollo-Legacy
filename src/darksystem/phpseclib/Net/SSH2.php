...
namespace darksystem\phpseclib\Net;

use darksystem\phpseclib\Crypt\Base;
use darksystem\phpseclib\Crypt\Blowfish;
use darksystem\phpseclib\Crypt\Hash;
use darksystem\phpseclib\Crypt\Random;
use darksystem\phpseclib\Crypt\RC4;
use darksystem\phpseclib\Crypt\Rijndael;
use darksystem\phpseclib\Crypt\RSA;
use darksystem\phpseclib\Crypt\TripleDES;
use darksystem\phpseclib\Crypt\Twofish;
use darksystem\phpseclib\Math\Integer;
use darksystem\phpseclib\System\SSH\Agent;
    ...
    function _key_exchange($kexinit_payload_server)
    {
        ...
        if ($kex_algorithm === 'curve25519-sha256@libssh.org') {
            ...
            if (strpos($kex_algorithm, 'diffie-hellman-group-exchange') === 0) {
                ...
                $prime = new Integer($primeBytes, -256);
        ...
        if ($kex_algorithm === 'curve25519-sha256@libssh.org') {
            ...
            $key = new Integer(\Sodium\crypto_scalarmult($x, $fBytes), 256);
            \Sodium\memzero($x);
        } else {
            $f = new Integer($fBytes, -256);
    ...
    function getServerPublicHostKey()
    {
        ...
        switch ($this->signature_format) {
            case 'ssh-dss':
                ...
                $temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $p = new Integer($this->_string_shift($server_public_host_key, $temp['length']), -256);

                if (strlen($server_public_host_key) < 4) {
                    return false;
                }
                $temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $q = new Integer($this->_string_shift($server_public_host_key, $temp['length']), -256);

                if (strlen($server_public_host_key) < 4) {
                    return false;
                }
                $temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $g = new Integer($this->_string_shift($server_public_host_key, $temp['length']), -256);

                if (strlen($server_public_host_key) < 4) {
                    return false;
                }
                $temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $y = new Integer($this->_string_shift($server_public_host_key, $temp['length']), -256);
                ...
                $r = new Integer($this->_string_shift($signature, 20), 256);
                $s = new Integer($this->_string_shift($signature, 20), 256);
                ...
                $u1 = $w->multiply(new Integer(sha1($this->exchange_hash), 16));
                list(, $u1) = $u1->divide($q);
            ...
            case 'ssh-rsa':
                ...
                $temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $e = new Integer($this->_string_shift($server_public_host_key, $temp['length']), -256);
                ...
                */
                ...
                $temp = unpack('Nlength', $this->_string_shift($signature, 4));
                $s = new Integer($this->_string_shift($signature, $temp['length']), 256);
                ...
                if ($s->compare(new Integer()) < 0 || $s->compare($n->subtract(new Integer(1))) > 0) {
                    user_error('Invalid signature');
                    return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
                }
