...
class RegisterCustomBlockTest extends Test{
	...
	public function run(){
		BlockFactory::init(true); //clear any previous test stuff

		$original = new MyCustomBlock();

		try{
			BlockFactory::registerBlock($original); //this ought to raise an exception because we haven't specified that we want to override Cobblestone
			throw new TestFailedException("No exception thrown ");
		}catch(\RuntimeException $e){
			//good
		}

		BlockFactory::registerBlock($original, true); //should be quiet

		$copy = BlockFactory::get(Block::COBBLESTONE);
